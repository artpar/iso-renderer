import { describe, it, expect } from 'vitest';
import { createCanvas } from 'canvas';
import { renderLabels } from './labels';
import { singleBuildingMap, threeBuildingsMap, twoCitiesWithRoadMap } from '../test/fixtures';
import { matchesSnapshot } from '../test/visual';
import type { Viewport, RenderState } from '../types';

const vp: Viewport = { width: 800, height: 600, zoom: 8, offsetX: 0, offsetY: 0 };
const state: RenderState = { selectedId: null, hoveredId: null, highlightedIds: new Set() };

describe('labels layer', () => {
  it('renders building labels', () => {
    const canvas = createCanvas(800, 600);
    const ctx = canvas.getContext('2d');
    const map = singleBuildingMap();

    ctx.fillStyle = '#1a1a1a';
    ctx.fillRect(0, 0, 800, 600);
    renderLabels(ctx as unknown as CanvasRenderingContext2D, map, vp, state);

    const result = matchesSnapshot('layer-labels-building', canvas);
    expect(result.pass).toBe(true);
  });

  it('renders city labels', () => {
    const canvas = createCanvas(800, 600);
    const ctx = canvas.getContext('2d');
    const map = twoCitiesWithRoadMap();

    ctx.fillStyle = '#1a1a1a';
    ctx.fillRect(0, 0, 800, 600);
    renderLabels(ctx as unknown as CanvasRenderingContext2D, map, { ...vp, zoom: 4 }, state);

    const result = matchesSnapshot('layer-labels-city', canvas);
    expect(result.pass).toBe(true);
  });

  it('hides labels below zoom threshold', () => {
    const canvas = createCanvas(800, 600);
    const ctx = canvas.getContext('2d');
    const map = threeBuildingsMap();

    ctx.fillStyle = '#1a1a1a';
    ctx.fillRect(0, 0, 800, 600);
    // Very zoomed out — labels should be hidden
    renderLabels(ctx as unknown as CanvasRenderingContext2D, map, { ...vp, zoom: 0.3 }, state);

    // Check that no text was drawn (canvas should still be background color)
    const data = ctx.getImageData(0, 0, 800, 600).data;
    let nonBgPixels = 0;
    for (let i = 0; i < data.length; i += 4) {
      if (data[i] !== 0x1a || data[i + 1] !== 0x1a || data[i + 2] !== 0x1a) {
        nonBgPixels++;
      }
    }
    expect(nonBgPixels).toBe(0);
  });
});
